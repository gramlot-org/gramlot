...
def page_files():
    ...
    pages.append(("catalog", RUNNER / "catalog" / "page.py", RUNNER / "catalog" / "page.js"))
...
def stage_pages(directory: Path, pages):
    ...
    for route, py_file, js_file in pages:
        ...
        (javascript_dir / f"{route}.js").write_text(f"export {{Page}} from {json.dumps(js_file.as_uri())};\n")
...
class RunnerApplication:
    def __init__(self, python_pages: Path, node_url: str, folders):
        ...
        allowed = {
            ...
            "/examples/00-runner/catalog/README.md": RUNNER / "catalog" / "README.md",
            "/examples/00-runner/catalog/page.py": RUNNER / "catalog" / "page.py",
            "/examples/00-runner/catalog/page.js": RUNNER / "catalog" / "page.js",
            "/examples/00-runner/catalog/style.css": RUNNER / "catalog" / "style.css",
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="gramlot-runner-") as temporary:
        ...
        node = subprocess.Popen(["node", str(RUNNER / "server.mjs")], cwd=ROOT,
                                env=env, stdout=subprocess.PIPE, text=True)
        try:
            node_url = node.stdout.readline().strip()
            if not node_url.startswith("http://127.0.0.1:"):
                raise RuntimeError("Node example host did not start")
