...
Current files/dependencies only. For current status use [GC-070](070-work-status.md) and for release execution use [GC-110](110-native-html-readiness.md#gc-110-020). GC-094 and the older ownership proposals are historical.

`genro-dom-js` remains a reference repository but
is no longer a dependency; Gramlot owns `GramlotRenderer` and `HtmlElement`.
Fresh package installation and Chromium 7/7 are recorded in GC-110 §3.1. [GC-087](087-javascript-layer-boundaries.md)
