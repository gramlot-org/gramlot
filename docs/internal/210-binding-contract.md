...
fired, level, registration. The root event path begins with main; remove only that
...
No operations.js: writes use Source node methods after upstream fixes. SET reacts,
PUT is silent (U1), FIRE emits even equal values with fired metadata (U2) and resets
to null silently. FIRE_AFTER (U3) has default 10 ms; Builder timer persists after
node removal while closed recipients are not delivered. Installation alone uses
