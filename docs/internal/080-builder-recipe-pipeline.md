...
[Concise counterpart](../../docs_llm/internal/080-builder-recipe-pipeline.md).

<a id="gc-080-005"></a>

## 005 · Contract and ownership

Python and JavaScript each expose a `GramlotBuilder` dialect over the corresponding
generic builder grammar. Python authors trusted page Source; JavaScript may author
local Source. Browser declarations are descriptions and never execute during Python
construction. Python rendering stays outside this browser pipeline.
...
The former main and remote Source payloads decoded into detached trees. The historical RecipeExpander resolved
all named markers there, including nested recipes, before the completed tree is
inserted once into observed application Source. Unknown recipes and recursive
expansion fail before insertion. Intermediate markers and partial expansions are
never rendered or exposed to Source subscribers.

Historically, recipes were supplied through the application constructor registry. Default host
bootstrap does not discover recipe modules; a custom runtime URL wrapper can
configure the registry. Automatic discovery remains a later contract.

This slice supports native HTML only. It does not translate color/background or
other CSS shorthand, add component inventory, execute Python data declarations,
or define formal Live Object Tree semantics.
